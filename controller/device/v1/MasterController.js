 ...
 * @return {obj} : created Master. {status, message, data}
 */ 
 ...
 * @return {obj} : found Master(s). {status, message, data}
 */
const findAllMaster = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      let totalRecords = await dbService.countDocument(Master, query);
    ...
    let result = await dbService.getAllDocuments( Master,query,options);
 ...
 * @param {obj} res : response contains updated documents of Master.
 * @return {obj} : number of deactivated documents of Master. {status, message, data}
 */
const softDeleteManyMaster = async (req,res) => {
  try {
    ...
    let result = await deleteDependentService.softDeleteMaster(query, updateBody);
    if (!result) {
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
 ...
 * @return {obj} : created Masters. {status, message, data}
 */
 ...
 * @param {obj} res : response of updated Masters.
 * @return {obj} : updated Masters. {status, message, data}
 */
 ...
 * @param {obj} res : response contains updated document of Master.
 * @return {obj} : deactivated Master. {status, message, data}
 */
 ...
 * @return {obj} : found Master. {status, message, data}
 */
const getMaster = async (req,res) => {
  try {
    ...
    let options = {};
    let result = await dbService.getSingleDocument(Master,query, options);
    if (result){
 ...
 * @return {obj} : deleted Master. {status, message, data}
 */
