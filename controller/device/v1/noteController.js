 ...
 * @return {obj} : created Note. {status, message, data}
 */ 
const addNote = async (req, res) => {
  try {
    ...
    let result = await dbService.createDocument(Note,data);
 ...
 * @return {obj} : found Note(s). {status, message, data}
 */
const findAllNote = async (req,res) => {
  try {
    ...
    let result = await dbService.getAllDocuments( Note,query,options);
...
const getNoteCount = async (req,res) => {
  try {
    ...
    let result = await dbService.countDocument(Note,where);
 ...
 * @param {obj} res : response contains updated documents of Note.
 * @return {obj} : number of deactivated documents of Note. {status, message, data}
 */
const softDeleteManyNote = async (req,res) => {
  try {
    ...
    let data = await dbService.bulkUpdate(Note,query, updateBody);
    if (!data) {
      return res.recordNotFound();
    }
 ...
 * @return {obj} : created Notes. {status, message, data}
 */
const bulkInsertNote = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkInsert(Note,data);
    return  res.ok({ data :result });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
 ...
 * @param {obj} res : response of updated Notes.
 * @return {obj} : updated Notes. {status, message, data}
 */
const bulkUpdateNote = async (req,res)=>{
  try {
    ...
    if ( typeof req.body.data === 'object' && req.body.data !== null) {
      ...
      let result = await dbService.bulkUpdate(Note,filter,data);
      if (!result){
        return res.recordNotFound();
      }
      return  res.ok({ data :result });
...
const deleteManyNote = async (req, res) => {
  try {
    ...
    const query = { '_id':{ '$in':ids } };
    let result = await dbService.deleteMany(Note,query);
    return res.ok({ data :result });
  } catch (error){
    return res.failureResponse(); 
  }
 ...
 * @param {obj} res : response contains updated document of Note.
 * @return {obj} : deactivated Note. {status, message, data}
 */
const softDeleteNote = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const updateBody = { isDeleted: true, };
    let result = await dbService.findOneAndUpdateDocument(Note, query, updateBody,{ new:true });
    if (!result){
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
...
const partialUpdateNote = async (req,res) => {
  try {
    ...
    let result = await dbService.findOneAndUpdateDocument(Note, query, data,{ new:true });
    if (!result) {
      return res.recordNotFound();
    }
    return res.ok({ data:result });
 ...
 * @return {obj} : found Note. {status, message, data}
 */
const getNote = async (req,res) => {
  try {
    ...
    let options = {};
    let result = await dbService.getSingleDocument(Note,query, options);
    if (result){
 ...
 * @return {obj} : deleted Note. {status, message, data}
 */
const deleteNote = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const result = await dbService.findOneAndDeleteDocument(Note, query);
    if (result){
      return  res.ok({ data :result });
    }
    return res.recordNotFound();
