...
const softDeleteManyTask = async (req,res) => {
  try {
    ...
    let data = await dbService.bulkUpdate(Task,query, updateBody);
    if (!data) {
      return res.recordNotFound();
    }
...
const softDeleteTask = async (req,res) => {
  try {
    ...
    const updateBody = { isDeleted: true, };
    let result = await dbService.findOneAndUpdateDocument(Task, query, updateBody,{ new:true });
    if (!result){
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
...
const deleteTask = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const result = await dbService.findOneAndDeleteDocument(Task, query);
    if (result){
      return  res.ok({ data :result });
    }
    return res.recordNotFound();
