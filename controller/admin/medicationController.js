 ...
 * @return {obj} : created Medication. {status, message, data}
 */ 
const addMedication = async (req, res) => {
  try {
    ...
    let result = await dbService.createDocument(Medication,data);
 ...
 * @return {obj} : found Medication(s). {status, message, data}
 */
const findAllMedication = async (req,res) => {
  try {
    ...
    let result = await dbService.getAllDocuments( Medication,query,options);
...
const getMedicationCount = async (req,res) => {
  try {
    ...
    let result = await dbService.countDocument(Medication,where);
 ...
 * @param {obj} res : response contains updated documents of Medication.
 * @return {obj} : number of deactivated documents of Medication. {status, message, data}
 */
const softDeleteManyMedication = async (req,res) => {
  try {
    ...
    let data = await dbService.bulkUpdate(Medication,query, updateBody);
    if (!data) {
      return res.recordNotFound();
    }
 ...
 * @return {obj} : created Medications. {status, message, data}
 */
const bulkInsertMedication = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkInsert(Medication,data);
    return  res.ok({ data :result });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
 ...
 * @param {obj} res : response of updated Medications.
 * @return {obj} : updated Medications. {status, message, data}
 */
const bulkUpdateMedication = async (req,res)=>{
  try {
    ...
    if ( typeof req.body.data === 'object' && req.body.data !== null) {
      ...
      let result = await dbService.bulkUpdate(Medication,filter,data);
      if (!result){
        return res.recordNotFound();
      }
      return  res.ok({ data :result });
...
const deleteManyMedication = async (req, res) => {
  try {
    ...
    const query = { '_id':{ '$in':ids } };
    let result = await dbService.deleteMany(Medication,query);
    return res.ok({ data :result });
  } catch (error){
    return res.failureResponse(); 
  }
 ...
 * @param {obj} res : response contains updated document of Medication.
 * @return {obj} : deactivated Medication. {status, message, data}
 */
const softDeleteMedication = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const updateBody = { isDeleted: true, };
    let result = await dbService.findOneAndUpdateDocument(Medication, query, updateBody,{ new:true });
    if (!result){
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
...
const partialUpdateMedication = async (req,res) => {
  try {
    ...
    let result = await dbService.findOneAndUpdateDocument(Medication, query, data,{ new:true });
    if (!result) {
      return res.recordNotFound();
    }
    return res.ok({ data:result });
 ...
 * @return {obj} : found Medication. {status, message, data}
 */
const getMedication = async (req,res) => {
  try {
    ...
    let options = {};
    let result = await dbService.getSingleDocument(Medication,query, options);
    if (result){
 ...
 * @return {obj} : deleted Medication. {status, message, data}
 */
const deleteMedication = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    const result = await dbService.findOneAndDeleteDocument(Medication, query);
    if (result){
      return  res.ok({ data :result });
    }
    return res.recordNotFound();
